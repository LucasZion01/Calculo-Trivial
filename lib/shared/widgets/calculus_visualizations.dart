...
abstract class _GraphPainter extends CustomPainter {
  ...
  void drawGrid(Canvas canvas, Size size) {
    ...
    for (var x = -3; x <= 3; x++) {
      canvas.drawLine(point(size, x.toDouble(), -1), point(size, x.toDouble(), 6), gridPaint);
    }
    for (var y = 0; y <= 6; y++) {
      canvas.drawLine(point(size, -3, y.toDouble()), point(size, 3, y.toDouble()), gridPaint);
    }
...
class _DerivativePainter extends _GraphPainter {
  ...
  @override
  bool shouldRepaint(covariant _DerivativePainter oldDelegate) => oldDelegate.x != x;
}
