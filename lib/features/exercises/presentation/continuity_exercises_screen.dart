...
class ContinuityExercisesScreen extends StatefulWidget {
  const ContinuityExercisesScreen({super.key});

  @override
  State<ContinuityExercisesScreen> createState() =>
      _ContinuityExercisesScreenState();
}

class _ContinuityExercisesScreenState extends State<ContinuityExercisesScreen> {
  ...
  void initState() {
    ...
    final selectedIds = AppProgress.selectExerciseQuestionIds(
      lessonId: AppProgress.continuityId,
      availableQuestionIds: mockContinuityExercises.map((exercise) => exercise.id),
    );

    final exercisesById = <String, ExerciseData>{
      for (final exercise in mockContinuityExercises) exercise.id: exercise,
    };
  ...
  void _finishPractice() {
    ...
    if (reviewItems.isEmpty) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => ContinuityFinalTestScreen(
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) => _ContinuityPracticeReviewScreen(
...
class _ContinuityPracticeReviewScreen extends StatefulWidget {
  final List<ExerciseReviewItem> reviewItems;
  final Set<String> practiceQuestionIds;
  final int correctAnswers;
  final int totalQuestions;

  const _ContinuityPracticeReviewScreen({
  ...
  @override
  State<_ContinuityPracticeReviewScreen> createState() =>
      _ContinuityPracticeReviewScreenState();
}

class _ContinuityPracticeReviewScreenState
    extends State<_ContinuityPracticeReviewScreen> {
  int currentIndex = 0;
