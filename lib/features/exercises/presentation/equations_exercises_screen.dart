...
import 'equations_final_test_screen.dart';
...
class _EquationsExercisesScreenState extends State<EquationsExercisesScreen> {
  ...
  void initState() {
    ...
    final selectedIds = AppProgress.selectExerciseQuestionIds(
      ...
      availableQuestionIds: mockEquationsExercises.map((exercise) => exercise.id),
    );

    final exercisesById = <String, ExerciseData>{
      for (final exercise in mockEquationsExercises) exercise.id: exercise,
    };
  ...
  Future<void> _finishPractice() async {
    ...
    final evidence = await ModuleMasteryTracker.recordPracticeResult(
      ...
      legacyCompleted: AppProgress.equationsAndInequationsCompleted,
    );
    ...
    if (reviewItems.isEmpty && decision.canTakeFinalTest) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => EquationsFinalTestScreen(
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) => _EquationsPracticeReviewScreen(
...
class _EquationsPracticeReviewScreen extends StatefulWidget {
  ...
  const _EquationsPracticeReviewScreen({
  ...
  @override
  State<_EquationsPracticeReviewScreen> createState() =>
      _EquationsPracticeReviewScreenState();
}

class _EquationsPracticeReviewScreenState
    extends State<_EquationsPracticeReviewScreen> {
  int currentIndex = 0;
