...
import 'derivatives_final_test_screen.dart';

class DerivativesExercisesScreen extends StatefulWidget {
  const DerivativesExercisesScreen({super.key});

  @override
  State<DerivativesExercisesScreen> createState() =>
      _DerivativesExercisesScreenState();
}

class _DerivativesExercisesScreenState
    extends State<DerivativesExercisesScreen> {
  ...
  void initState() {
    ...
    final selectedIds = AppProgress.selectExerciseQuestionIds(
      ...
      availableQuestionIds: mockDerivativesExercises.map((exercise) => exercise.id),
    );

    final exercisesById = <String, ExerciseData>{
      for (final exercise in mockDerivativesExercises) exercise.id: exercise,
    };
  ...
  void _finishPractice() {
    ...
    if (reviewItems.isEmpty) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => DerivativesFinalTestScreen(
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) => _DerivativesPracticeReviewScreen(
...
class _DerivativesPracticeReviewScreen extends StatefulWidget {
  final List<ExerciseReviewItem> reviewItems;
  final Set<String> practiceQuestionIds;
  final int correctAnswers;
  final int totalQuestions;

  const _DerivativesPracticeReviewScreen({
  ...
  @override
  State<_DerivativesPracticeReviewScreen> createState() =>
      _DerivativesPracticeReviewScreenState();
}

class _DerivativesPracticeReviewScreenState
    extends State<_DerivativesPracticeReviewScreen> {
  int currentIndex = 0;
